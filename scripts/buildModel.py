...
def getIndex(p: UVCharacteristics):
    ...
    INDEXRANGES = {100:1, 1000:4, 400:2, 780:3, 40:0, 1800:5}
    
    out = min((map(errorCalc, [(x,p.mean) for x in INDEXRANGES.keys()])))
    return INDEXRANGES[out[1]]

def correctPolarized(p, data):
    if p.polarized:
        data = [i*2.2 for i in data]
        p.polarized = False
        p.mean = np.mean(data)
    return (p,data)
...
def buildTrainingSet(location):
    ...
    for file in os.listdir(directory):
        if os.fsdecode(file).startswith('polarized'):
            ...
            indexParam, data = correctPolarized(indexParam, data)
...
def forceUniformity(data):
    ...
    for i in range(len(data)):
        if len(data[i]) > maxDataSize:
            data[i] = [random.choice(data[i]) for _ in range(maxDataSize)]
